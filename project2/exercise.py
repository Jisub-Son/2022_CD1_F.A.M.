...
class EXERCISE(KEYPOINT): ## 
    ...
    def pushup(detect, counter, status, set, feedback, count): ## 푸쉬업
        left_arm_angle = detect.angle_of_the_left_arm()
        right_arm_angle = detect.angle_of_the_left_arm()
    ...
    def squat(detect, counter, status, set, feedback, count): ## 스쿼트
        left_leg_angle = detect.angle_of_the_right_leg() ## 무릎각도
        right_leg_angle = detect.angle_of_the_left_leg()
    ...
    def calculate_exercise(detect, exercise_type, counter, status, set, feedback, count): ## 운동횟수 계산
        if exercise_type == "pushup":
            counter, status, set, feedback, count = EXERCISE(detect.landmarks).pushup(
                counter, status, set, feedback, count)
        elif exercise_type == "squat":
            counter, status, set, feedback, count = EXERCISE(detect.landmarks).squat(
                counter, status, set, feedback, count)
        return [counter, status, set, feedback, count]
