...
from threadingCam2 import stateInfo
import cv2
import time
import numpy as np
...
if __name__ == '__main__':
    ...
    lndmkPipe_child, lndmkPipe_parent = Pipe()
    
    shm = shared_memory.SharedMemory(create=True, size=1000)
    
    proc_get0 = GetVideo(LEFT_CAM, getPipe_child0, shm.name)
    ...
    # if getPipe_parent0.recv() == 'show start' and getPipe_parent1.recv() == 'show start':
        # print('ok')
    if getPipe_parent0.recv() == 'show start':
        print('ok')
