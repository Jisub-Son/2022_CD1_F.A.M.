...
class EXERCISE(KEYPOINT):
    ...
    def squat(self, reps, status, sets, feedback, timer, camID):
        ...
        KNEEDOWN_CONDITION = (feedback != 'Place your knees behind toes' and feedback != 'start exercise' and feedback != 'Parallel your feet' and status != 'Rest')
        LESSDOWN_CONDITION = (status == 'Down' and feedback != 'Bend your legs less' and feedback != 'Place your knees behind toes' and status != 'Rest')
        PARALLEL_CONDITION = (feedback != 'Parallel your feet' and feedback != 'Place your knees behind toes' and feedback != 'start exercise' and status != 'Rest')
        DEFAULT_CONDITION = (feedback != 'Success' or status == 'Down')
        
        COUNT_ANGLE = (LESS_LEG_ANGLE < avg_leg_angle < REF_LEG_ANGLE and avg_knee_angle > REF_KNEE_ANGLE and LESS_PARALLEL_ANGLE < avg_foot_parallel < MORE_PARALLEL_ANGLE)
        MOREDOWN_ANGLE = (REF_LEG_ANGLE < avg_leg_angle < MORE_LEG_ANGLE and avg_knee_angle > REF_KNEE_ANGLE and LESS_PARALLEL_ANGLE < avg_foot_parallel < MORE_PARALLEL_ANGLE)
        KNEEDOWN_ANGLE = (avg_knee_angle < REF_KNEE_ANGLE)
        LESSDOWN_ANGLE = (avg_leg_angle < LESS_LEG_ANGLE and avg_knee_angle > REF_KNEE_ANGLE and LESS_PARALLEL_ANGLE < avg_foot_parallel < MORE_PARALLEL_ANGLE)
        PARALLEL_ANGLE = (avg_foot_parallel < LESS_PARALLEL_ANGLE or avg_foot_parallel > MORE_PARALLEL_ANGLE)
        DEFAULT_ANGLE = (avg_leg_angle > MORE_LEG_ANGLE and avg_knee_angle > REF_KNEE_ANGLE and LESS_PARALLEL_ANGLE < avg_foot_parallel < MORE_PARALLEL_ANGLE)
        ...
        if camID == LEFT_CAM:
            ...
            if COUNT_CONDITION and COUNT_ANGLE:    # 적절하게 구부렸을 때
                    ...
                    feedback = 'Success'
                    prev = time.time()
                    color = [(255, 0, 0), (255, 0, 0), (255, 0, 0)]      # leg = blue, knee = blue          
            else:   # 기본 자세가 충족된 상태에서 무릎을 구부릴 때
                if  MOREDOWN_CONDITION and MOREDOWN_ANGLE:  # 너무 적게 구부렸을 때
                    voiceFeedback('moredown')
                    status = 'Up'
                    feedback = 'Bend your legs more'
                    color = [(0, 0, 255), (0, 0, 0), (0, 0, 0)]
                elif KNEEDOWN_CONDITION and KNEEDOWN_ANGLE: # 기존 자세 충족 안됨 -> 무릎이 발끝보다 앞에 있을 때
                    voiceFeedback('kneedown')
                    status = 'Up'
                    feedback = 'Place your knees behind toes'
                    color = [(0, 0, 0), (0, 0, 255), (0, 0, 0)]    
                elif LESSDOWN_CONDITION and LESSDOWN_ANGLE: # 너무 많이 구부렸을 때
                    voiceFeedback('lessdown')
                    reps -= 1
                    status = 'Up'
                    feedback = 'Bend your legs less'
                    color = [(0, 0, 255), (0, 0, 0), (0, 0, 0)]     
                elif PARALLEL_CONDITION and PARALLEL_ANGLE: ## 발 11자 아닐때
            ...
            if reps == REF_REPS and status == 'Up':
                if timer == REF_TIMER and feedback == 'Start' and camID == 0:
