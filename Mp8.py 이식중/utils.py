...
def table(exercise, reps, status, sets, feedback, timer): 
    ...
    cv2.putText(table, "Exercise            " + str(exercise), (1, 95), ## opencv문자열: table 운동타입(입력한 운동타입)
...
def table_calculations(*exercise, **kwexercise):
    table_calculations = cv2.imread("table\./table_angle.PNG")
    for i, key in enumerate(kwexercise):
        cv2.putText(table_calculations, "{} : {:.4f}".format(key, kwexercise[key]), (1, 150 + i*90), ## opencv문자열: table 운동 카운트
                    cv2.FONT_HERSHEY_SIMPLEX, 1, exercise[0][i], 2, cv2.LINE_AA) ## 문자열: 위치, 크기, 색상(검정) 설정
