...
class EXERCISE(KEYPOINT):
    ...
    # timer function
    def Rest_timer(self, exercise, reps, status, sets, feedback, timer):
    ...
    # squat function
    def squat(self, exercise, reps, status, sets, feedback, timer, src):
        ...
        if src == LEFT_CAM: ## cam1
            ...
            if status == 'Rest':
                exercise, reps, status, sets, feedback, timer = self.Rest_timer(exercise, reps, status, sets, feedback, timer)  # run timer function
        ...
        return [exercise, reps, status, sets, feedback, timer, src]

    # pushup function
    def pushup(self, exercise, reps, status, sets, feedback, timer, src):
    ...
    def sidelateralraise(self, exercise, reps, status, sets, feedback, timer, src):
        ...
        if src == LEFT_CAM: ## 노트북
            ...
            # count logic
            if ELBOW_ANGLE and PARALLEL_RATIO:         # 기본 자세가 만족되고(팔꿈치가 적절히 구부려짐)
                ...
                if feedback == 'Success' and (not ELBOW_ANGLE or not PARALLEL_RATIO):  # 카운트가 된 직후 잘못 자세를 잡았을 때
                    reps -= 1
                    status = 'Down'
                    feedback = 'Keep your position to the end'
                    color = [(0, 0, 0), (0, 0, 255), (0, 0, 255)]
    ...
    # select mode
    def calculate_exercise(self, exercise, reps, status, sets, feedback, timer, src): 
        if exercise == "pushup":
            print('exercise = pushup')
            reps, status, sets, feedback, timer, src = EXERCISE(self.landmarks).pushup(
                reps, status, sets, feedback, timer, src)
        elif exercise == "squat":
            print('exercise = squat')
            reps, status, sets, feedback, timer, src = EXERCISE(self.landmarks).squat(
                reps, status, sets, feedback, timer, src)
        elif exercise == "sidelateralraise":
            print('exercise = sidelateralraise')
            reps, status, sets, feedback, timer, src = EXERCISE(self.landmarks).sidelateralraise(
                reps, status, sets, feedback, timer, src)    
        
        return [reps, status, sets, feedback, timer, src]