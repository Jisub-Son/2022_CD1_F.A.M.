    ...
    def AddMp(self):                
        ...
        with mp_pose.Pose(min_detection_confidence=0.5,         # 최소감지신뢰값( [0.0, 1.0] ) 기본값 = 0.5
            ...
            while not self.stopped:
                ...
                results2 = pose.process(self.frame2)
                ...
                cv2.imshow("video0", self.frameBuf1)
                cv2.imshow("video1", self.frameBuf2)
...
def threadAll(src1=0, src2=1):              # 3개 쓰레드를 모두 실행시켰을 경우
    ...
    while True:
        if video_getter.stopped or video_adder.stopped:
            video_adder.stop()
            video_getter.stop()
            break
        ...
        frame1 = putIterationsPerSec(frame1, cps.countsPerSec())
        frame2 = putIterationsPerSec(frame2, cps.countsPerSec())     # 스켈레톤 붙은 frame에 텍스트 넣기
        video_adder.frame1 = frame1
        video_adder.frame2 = frame2             # 읽은 frame을 mpThread에 보내기
        cps.increment()
...
# line 188 imshow 추가
# getVideoThread -> AddMediapipeThread(여기서 imshow) 구조
# iteration 값 12000(노트북), 스켈레톤 불안정